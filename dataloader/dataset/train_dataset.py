...
class Dataset(Dataset):
    def __init__(self, config: BaseConfig, mode: Literal["train", "valid"]):
        ...
        if self.mode == 'train':
            text, audio, vision, self.labels['M'], self.ids, self.audio_lengths, self.vision_lengths = \
            text[:self.config.train_size], audio[:self.config.train_size], vision[:self.config.train_size], self.labels['M'][:self.config.train_size], self.ids[:self.config.train_size], self.audio_lengths[:self.config.train_size], self.vision_lengths[:self.config.train_size]
